...
def generate_full_app_code(prompt, language):
    ...
    contents = [
        types.Content(
            role="user",
            parts=[types.Part.from_text(text=f"Generate a full production-ready {language} application project based on this description: {prompt}. Include all necessary files, Docker config, tests, deployment scripts, and Supabase integration (edge functions, database, auth).")],
        )
    ]
...
def deploy_supabase_edge_function(supabase_url, supabase_key, function_name, code_text):
    ...
    try:
        ...
        if response.status_code == 200 or response.status_code == 201:
            return True, "✅ Edge function deployed successfully."
        else:
            return False, f"❌ Deployment failed: {response.text}"
    except Exception as e:
        return False, f"⚠️ Error deploying function: {str(e)}"

# Streamlit App Interface
def main():
    st.set_page_config(page_title="Gemini + Supabase App Generator", layout="wide")

    st.title("🚀 Gemini + Supabase End-to-End App Generator with Edge Functions")
    ...
    if st.button("💡 Generate Full App Code"):
        if not user_prompt.strip() or not supabase_url or not supabase_key:
            st.error("⚠️ Please provide your Supabase URL, API key, and application description.")
            return

        with st.spinner("⏳ Generating full production-ready code..."):
            try:
    ...
    uploaded_file = st.file_uploader("📂 Upload existing project ZIP", type="zip")
    if uploaded_file:
        try:
            ...
            if supabase_url and supabase_key:
                with open(os.path.join(extracted_path, "app_description.txt"), "r") as desc_file:
            ...
            st.error(f"⚠️ Upload/Extraction failed: {str(e)}")
